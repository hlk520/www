...
//						index=-$(this).index();
//					}
					localStorage.setItem(id+'link',index);
			        if(isClick){
				...
				var firstLink;
				var firstTitle;
				if(!localStorage.getItem(id+'link')){
					localStorage.setItem(id+'link','0');
				}
				$.getJSON("http://query.yahooapis.com/v1/public/yql",
					{ q: "select * from json where url='http://api.zhuishushenqi.com/mix-atoc/"+id+"?view=chapters"+"'",format: "json"},
					function(data){
						var dt=data.query.results.json.mixToc.chapters;
						var eq1=parseInt(localStorage.getItem(id+'link'));
						console.log(eq1)
						var lenth=dt.length;
						if(eq1<0){
							eq1=lenth+eq1;
						}
						firstLink=dt[eq1].link;
						firstTitle=dt[eq1].title;
...
//				$('#conter').hide().siblings('#link').show();
			...
			//返回主页
//			$('#top1>span:last').on('touchend',function(){
//			})
			//点击短时间变色
            ...
            $('.xzz').on('touchend',function(){
            	...
            	localStorage.setItem(id+'link',eq1)
            	var lin=$('#list').find('li').eq(eq1).attr('data-link');
				var hd=$('#list').find('li').eq(eq1).find('span').text();
				lin=encodeURIComponent(lin);//编码
					getContent(lin,hd)
            })
            //上一章
